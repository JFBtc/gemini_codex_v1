...
class BotController:
    def __init__(self):
        ...
        # --- NOUVEAUX ATTRIBUTS POUR LA V4 ---
        self.active_symbol = None  # Symbole regardé par l'utilisateur (pour le DataLab)
        self.dom_watched_levels = {} # Stockage des niveaux cochés pour le DOM {sym: [levels]}
    ...
    # --- GESTION DOM LEVELS (Sélecteur Niveaux) ---
    def set_dom_levels(self, symbol, levels):
        """Met à jour la liste des niveaux à afficher sur le DOM pour un symbole"""
        self.dom_watched_levels[symbol] = levels

    def get_dom_levels(self, symbol):
        """Récupère les niveaux validés par l'utilisateur"""
        return self.dom_watched_levels.get(symbol, [])
    ...
    # Ordre Limite (Bracket) - Utilisé par le clic DOM
    def place_limit_order(self, symbol, action, price, qty, sl_ticks, tp_ticks):
        if not self.guardian.validate_order(symbol, action, qty): return
        
        contract = self.contracts_map.get(symbol)
        if not contract: return
        tick_size = self.get_tick_size(symbol)
        
        # Calcul SL/TP relatifs au prix Limite
        if action == "BUY":
            sl_price = price - (sl_ticks * tick_size)
            tp_price = price + (tp_ticks * tick_size)
        else:
            sl_price = price + (sl_ticks * tick_size)
            tp_price = price - (tp_ticks * tick_size)
        ...
        log.info(f"🛒 LMT {action} {symbol} @ {price} (x{qty}) [SL {sl_price} / TP {tp_price}]")
        
        try:
            orders = self.ib_manager.ib.bracketOrder(action, qty, limitPrice=price, takeProfitPrice=tp_price, stopLossPrice=sl_price)
            for o in orders: 
                o.outsideRth = True
                o.tif = 'GTC'
            for o in orders: self.ib_manager.ib.placeOrder(contract, o)
        except Exception as e:
            log.error(f"❌ Erreur envoi ordre limite {symbol} : {e}")