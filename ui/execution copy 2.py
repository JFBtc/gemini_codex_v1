...
class ContextPanel(tk.Frame):
    ...
    def _update_content(self):
        ...
        # 1. INDICATEURS ACTIVÉS DANS DATALAB
        for key, cfg in settings.items():
            if not cfg.get("bias", False): continue
            
            # RSI
            if "RSI" in key:
                tf = key.split("_")[1] # ex: RSI_M15 -> M15
                data = radar.get(tf, {})
                rsi = data.get("rsi", 50)
                w = WEIGHTS.get(tf, 1)
        ...
        # 2. PATTERNS (Filtrés par Settings)
        for tf in ["M1", "M5", "M15", "H1", "H4"]:
            # On vérifie si les patterns de ce TF sont cochés pour le BIAS
            pat_key = f"PAT_{tf}"
            if not self._is_enabled(settings, pat_key, "bias"): 
                continue
        ...
        # 1. Niveaux Session (Filtrés par Settings)
        ...
        # 2. FVGs (Tous les TF dynamiquement)
        for tf, data in radar.items():
            ...
            for fvg in fvgs:
                ...
                if use_map:
                    ...
                    if is_inside:
                        dist = 0; d_txt = "INSIDE"; tag = "active"
                    else:
                        dist = min(abs(last_px - fvg['top']), abs(last_px - fvg['bot']))
                        d_txt = f"{dist:.2f}"
...
class ExecutionView(tk.Frame):
    ...
    def _on_change_sym(self, event=None):
        ...
        # --- SYNCHRONISATION DATALAB ---
        # On définit le symbole actif dans le contrôleur pour que le Lab sache quoi afficher
