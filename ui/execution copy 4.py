...
class ContextPanel(tk.Frame):
    ...
    def _is_enabled(self, settings, key, feature="act"):
        # Si pas de settings (premier lancement), on active tout par défaut
        if not settings: return True
        # Si la clé n'existe pas, on active par sécurité
        if key not in settings: return True
    ...
    def _update_content(self):
        ...
        for tf in ["M1", "M5", "M15", "H1", "H4"]:
            ...
            for p in pats:
                # Inside Bar = Neutre (pas d'impact score)
                if p["side"] == "BULL":
                    score += (w * 1.5)
        ...
        for k, v in sess.items():
            ...
            # DOM List
            if is_checked:
                l_type = "sup" if last_px > v else "res"
                dom_export_list.append({"price": v, "type": l_type, "label": k})
            ...
            if self._is_enabled(settings, s_key, "map"):
                ...
                all_map_rows.append({
                    "dom": chk_char,
                    "desc": k.upper() + (f" ({v:+.2f})" if "Gap" in k else ""), 
                    "lvl": "---" if "Gap" in k else f"{v:.2f}",
                    "dist": 9999 if "Gap" in k else abs(dist), 
                    "sort_dist": 9999 if "Gap" in k else abs(dist),
                    "tags": tuple(row_tags)
                })

        # VWAP
        if vwap:
            if self._is_enabled(settings, "VWAP", "mag"):
                valid_magnets.append((vwap, "VWAP", "session"))
            if self._is_enabled(settings, "VWAP", "map"):
                unique_id = "SESS_VWAP"
                is_checked = unique_id in self.checked_for_dom
                chk_char = "☑" if is_checked else "☐"
                if is_checked: dom_export_list.append({"price": vwap, "type": "sup" if last_px > vwap else "res", "label": "VWAP"})
                row_tags = [unique_id, "session"]
                if is_checked: row_tags.append("checked")
                all_map_rows.append({"dom": chk_char, "desc": "VWAP", "lvl": f"{vwap:.2f}", "dist": abs(last_px-vwap), "sort_dist": abs(last_px-vwap), "tags": tuple(row_tags)})
        ...
        for tf, data in radar.items():
            ...
            if fvgs:
                use_mag = self._is_enabled(settings, fvg_key, "mag")
                use_map = self._is_enabled(settings, fvg_key, "map")
                
                if use_mag or use_map:
                    for fvg in fvgs:
                        if fvg.get("mitigated", False): continue
                        
                        unique_id = f"FVG_{tf}_{fvg['bot']}" # ID simple
                        ...
                        if use_map:
                            ...
                            all_map_rows.append({
                                ...
                                "dist": dist, "sort_dist": dist,
                                "tags": tuple(row_tags)
                            })
                            
                            if is_checked:
                                # On envoie les bornes au DOM
                                l_type = "sup" if fvg['type'] == "BULL" else "res"
                                dom_export_list.append({"price": fvg['top'], "type": l_type, "label": f"{tf} FVG Top"})
                                dom_export_list.append({"price": fvg['bot'], "type": l_type, "label": f"{tf} FVG Bot"})

        # 3. PATTERNS (Reconnectés)
        for tf, data in radar.items():
            if tf == "SESSION": continue
            pat_key = f"PAT_{tf}"
            pats = data.get("patterns", [])
            ...
            for p in pats:
                ...
                unique_id = f"{tf}_{name_short}_{lvl}" 
                ...
                if use_map:
                    ...
                    if is_checked:
                        l_type = "sup" if p["side"] == "BULL" else "res"
                        dom_export_list.append({"price": lvl, "type": l_type, "label": f"{tf} {name_short}"})
